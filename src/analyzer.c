...
static float compute_for_core(proc_stat_core_info_t* core_info)
{
    ...
    long long idlealltime   = core_info->idle + core_info->iowait;
    long long systemalltime = core_info->system + core_info->irq + core_info->softirq;
    long long virtalltime   = core_info->guest + core_info->guest_nice;

    long long total = core_info->user + core_info->nice + systemalltime + idlealltime + core_info->steal + virtalltime;

    double busy_time = (double)total - (double)idlealltime;
    double busy_perc = busy_time / (double)total;

    return (float)(busy_perc * 100.f);
}
